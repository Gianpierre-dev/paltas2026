...
export default async function FundosListPage() {
  const session = await getServerSession();
  if (!session) redirect('/login');
  if (session.usuario.rol !== Rol.ADMIN) redirect('/dashboard');

  const items = await api<Fundo[]>('/fundos', {
    ...
    <div className="px-4 py-4 space-y-4">
      <header className="flex items-center justify-between gap-3">
        <div>
          ...
          <h2 className="text-xl font-semibold text-zinc-900 mt-0.5">Fundos</h2>
        </div>
        <Link
          href="/admin/catalogos/fundos/nuevo"
      ...
      <CatalogoTable
        ...
        headers={['Nombre']}
        rows={rows}
        emptyMessage="Aún no hay fundos cargados."
      />
    </div>
  );
}
