...
export default async function VariedadesListPage() {
  const session = await getServerSession();
  if (!session) redirect('/login');
  if (session.usuario.rol !== Rol.ADMIN) redirect('/dashboard');

  const items = await api<Variedad[]>('/variedades', {
  ...
  return (
    <div className="px-4 py-4 space-y-4">
      <header className="flex items-center justify-between gap-3">
        ...
        <Link
          href="/admin/catalogos/variedades/nuevo"
      ...
      <CatalogoTable
        ...
        headers={['Nombre']}
        rows={rows}
        emptyMessage="Aún no hay variedades cargadas."
      />
    </div>
  );
}
